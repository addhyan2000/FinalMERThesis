...
\textbf{No study isolates the backbone in a matched pair.} The corpus does compare descriptor-only methods against learned ones on the same folds, so it is not silent on whether a learned stem helps at all. What it does not contain is a comparison in which the stem is the \textit{only} thing that changes: each paper proposes a backbone and evaluates the whole system around it, so a reported gain cannot be attributed to the stem rather than to the representation, training schedule or corpus subset that changed with it.
