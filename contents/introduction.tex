...
\begin{enumerate}
	...
	\item \textbf{A measured account of the contribution of temporal and motion modelling.} Temporal modelling carries almost all of the measured effect: the temporal transformer improves pooled macro F1 by $+0.2173$ on average, and all six of its matched pairs are positive. The remaining components contribute far less. Eulerian video magnification has a small mean effect ($+0.0152$) whose direction varies across pairs; parameter-free attention has a mean effect ($+0.0034$) that cannot be distinguished from noise at this sample size; and the convolutional spatial stem, which consumes most of the computation in the study, is the only component with a negative mean effect ($-0.0310$).
	...
	\item \textbf{A documented account of excluded evaluations.} Four earlier evaluations of the same configurations are reported together with the property that disqualifies each of them, so that the results rest on one complete protocol rather than on the most favourable of several.
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Chapter 2 --- Background} sets out the mechanisms the thesis depends on, in the order the pipeline applies them: the phenomenon itself, what the recording supplies, magnification and motion representation, neural-network fundamentals and the blocks built from them, training under class imbalance, and evaluation on a small dataset.
	...
	\item \textbf{Chapter 6 --- Conclusion} states the contributions, the limits within which they hold, the work the design leaves undone, and the principal conclusions of the study.
\end{itemize}
