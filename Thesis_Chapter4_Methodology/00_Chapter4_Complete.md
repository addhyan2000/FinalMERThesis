...
Before reaching any of the four ablatable components described in §4.1.2, each of the 156 clips is reduced to a single three-channel motion tensor of fixed shape $[3, 32, 224, 224]$: 32 temporal frames, each $224 \times 224$, with the three channels holding horizontal optical flow, vertical optical flow, and optical strain magnitude respectively. The procedure by which a variable-length sequence of cropped greyscale frames is converted into this fixed-shape tensor — frame selection, flow computation, strain derivation, and the role of motion magnification in producing the flow the tensor encodes — is described in Chapters 2 §2.2 and §2.4, and is not re-derived here. The values that procedure was run with are recorded here, since they are fixed throughout the study and are needed to reproduce it. Magnification, where a configuration enables it, uses an amplification factor of $\alpha = 10$, a temporal band of 5–25 Hz and four pyramid levels, with the amplified output clipped to $[0,255]$ and requantised to 8-bit (§2.3.5 explains the band and §3.2.7 records the divergence it embodies). The frames spanning onset to offset are uniformly resampled to 33, and dense optical flow is computed between adjacent pairs with OpenCV's Farnebäck implementation at pyramid scale 0.5, three pyramid levels, window size 15, three iterations, polynomial neighbourhood 5 and polynomial smoothing 1.2, over 8-bit greyscale frames; strain follows from each field's spatial gradients by central differences at unit pixel spacing. None of these values is varied by the ablation. What matters for this chapter is only that every one of the 156 clips, regardless of which of the twelve valid ablation configurations subsequently processes it, enters the network as a tensor of this same fixed shape, so that no configuration in §4.1's design receives an input representation that differs from any other's.
...
**No kernel in this stem spans the temporal axis.** Every convolution has kernel depth 1, and the pooling operation is `(1,2,2)` — depth 1, spatial stride 2 in both remaining dimensions. Consequently the sequence length $T=32$ is unchanged through the entire stem, while height and width are halved once, from $224$ to $112$. Despite being built from `Conv3d` operations, this component is a purely spatial feature extractor applied independently, frame by frame: it computes a 2-D convolution replicated across all 32 time steps, not a spatio-temporal one. §2.6.1 describes what three-dimensional convolution is in general; the point recorded here is what this particular configuration of it does, which is exactly a per-frame spatial transform.
...
where $\mu$ and $\sigma^2$ are the mean and (Bessel-corrected, dividing by $n = D \cdot H \cdot W - 1$) variance taken over the spatio-temporal extent of that neuron's channel, and gates the feature map elementwise with $x \cdot \operatorname{sigmoid}(e)$. No weight or bias is learned anywhere in this computation, so SimAM introduces **zero learnable parameters** (confirmed in the totals of §4.4.9) regardless of how many streams or channels it is applied to. The regularisation constant is fixed at $\lambda = 1 \times 10^{-4}$ throughout, matching `SimAM3D`'s default and the value passed at construction.
...
The design intent behind both fallbacks is the same: each adds as little capacity as the pipeline's shape constraints allow, so that disabling a component isolates that component's contribution rather than confounding it with a change in the size or expressiveness of whatever replaces it.
...
This thesis's primary reported metric is **pooled macro F1** — the mean of the three values in the stored `per_class_f1` array. **The code never computes or stores this quantity under any key.** It exists nowhere in `final_results.json`, `summary.csv`, or any other artefact; it must be derived, after the fact, by averaging the three `per_class_f1` numbers. A reader expecting the headline number under a field called `macro_f1` will instead find the per-fold-averaged quantity of §4.6.3 — a different estimator, addressed next — and will not find pooled macro F1 recorded anywhere.
