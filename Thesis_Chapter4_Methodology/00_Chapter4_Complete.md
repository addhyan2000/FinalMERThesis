...
The dataclass default of 2 was therefore not the value used. Which value *was* used cannot be recovered with certainty from the artefacts, and this is recorded here rather than glossed. The override above places `*sys.argv[1:]` after its own `--batch_size 4`, so any value supplied further up the chain wins under `argparse`; the graphical launcher does supply one, appending `--batch_size` from `gui_settings.json` (`tools/run_gui.py`), where the stored value is **8** alongside a protocol, fold and epoch profile matching the runs of record exactly. No result file records the batch size, and no log survives that would settle it. The study therefore ran at 4 or at 8 — whichever launcher produced the stored results — and every statement in this thesis that depends on batch size should be read against that uncertainty.
