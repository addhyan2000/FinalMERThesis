...
SimAM's mean marginal contribution is **+0.0034** pooled macro F1, computed across **four** matched pairs — positive in **three**, negative in one. Individual differences range from **−0.0288** to **+0.0341**. This is the smallest mean effect of the four components ablated in this thesis, and at this sample size it is statistically indistinguishable from zero: with no variance estimate across seeds (§4.1.5), a mean of +0.0034 over four pairs gives no basis for separating a real small effect from noise.
