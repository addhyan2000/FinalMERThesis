...
This section reports the outcome of the twelve-configuration factorial sweep specified in §4.1 and §4.3, scored under the leave-one-subject-out protocol of §4.6. It gives the complete ranking, states the headline result plainly, and establishes the reference points against which every subsequent section reads its numbers. §5.2 through §5.4 return to the same twelve rows to isolate each component's marginal effect; this section only establishes what was found, not why.
...
This is not reversed by looking at accuracy instead. `config_8` does lead on pooled accuracy, at **0.7500** against `config_2`'s **0.7436**, but the margin is **one clip** out of 156 — 117 correct against 116. The two configurations are therefore two different winners under two different metrics: `config_2` wins decisively on pooled macro F1, and `config_8` wins by a single classification on pooled accuracy. §4.6.5 already established why this thesis treats pooled macro F1, not accuracy, as primary — mean-of-folds macro F1 carries a hard ceiling near 0.627 imposed by fold composition, and accuracy alone rewards majority-class prediction on an imbalanced corpus. Given that commitment, the ranking in Table 5.1 stands: `config_2_temporal_only`, not the proposed unified system, is the best-performing configuration in this study. Why this occurs — and what it implies for the four components under test — is taken up in §5.5 and §5.8; it is not explained away here.
...
This count requires care. `config_13_permutation` scores **0.4480**, which is above the baseline despite being, by every other measure in Table 5.1, a weak configuration — it ranks seventh of twelve and sits close to the cluster of transformer-free scores. It is tempting to group `config_13` with the four configurations named above and report five below the baseline; the correct figure, against `config_4`, is four. The choice of reference point also matters in its own right: measured instead against `config_1_pure_base` (0.4337), the set of configurations scoring lower is different, since `config_4` itself (0.4386) and `config_13` (0.4480) both then lie above it, while `config_3`, `config_5`, and `config_16` remain below. This thesis measures against `config_4`, as §4.3.4 specifies, and the count of four is reported on that basis throughout.
...
§4.3.5 fixes each component's marginal effect as the mean of its matched-pair differences in pooled macro F1, the construction §5.2 through §5.4 already apply to the transformer, motion magnification, and SimAM. Of the four components ablated in this thesis, the convolutional stem is the only one whose mean effect is negative. This section reports that result directly, sets it against what the stem costs to include, and states plainly what can and cannot be concluded from it.

### 5.5.1 The effect

The convolutional stem's mean marginal contribution is **−0.0310** pooled macro F1, computed across **four** matched pairs, positive in only **two** of the four. Individual differences range from **−0.1292** to **+0.0094**. No other component in this study has a negative mean effect: the transformer's is +0.2173 (§5.2.1), motion magnification's is +0.0152 (§5.3.1), and SimAM's is +0.0034 (§5.4.1). The component that adds the most machinery to the pipeline is the one component that, on average, makes it worse.

### 5.5.2 The four pairs

Table 5.6 lists all four matched pairs in which the stem is toggled with every other factor held fixed. The comparison is measured over four pairs rather than six for the same architectural reason noted for SimAM in §5.4.2: §4.1.3 excludes, as undefined, any configuration in which SimAM is active with no stem for it to act on, so the two cells that would have completed the matrix were never run.
...
The conclusion follows directly and should be stated at the strength the data support: **the component consuming 96.6% of this study's compute has a mean effect of −0.0310 pooled macro F1.** That is a finding about the specific pipeline built and measured in this thesis — a particular stem, at a particular resolution, feeding a particular downstream encoder — and not a general claim that convolutional stems are unhelpful for micro-expression recognition as such.
...
### 5.5.5 Interpretation, grounded and bounded
...
**The stem is spatial, not spatio-temporal.** §4.4.3 records that no kernel in `SingleStream3DCNN` spans the temporal axis: every convolution has kernel depth 1, and the pooling operation is `(1,2,2)` — spatial only. The sequence length of 32 passes through the stem unchanged. Despite being built from `Conv3d` operations, the component is a learned spatial feature extractor applied independently to each frame, not a mechanism that mixes information across time. Whatever the stem contributes, it cannot be a temporal contribution.
...
**The stem is large relative to the input it is asked to fit.** §2.2.3 and §3.6 record that this pipeline feeds the stem 224×224 frames, more than twice the 100×100 threshold above which the reviewed literature reports degrading performance for deeper backbones, and eight times the 28×28 input STSTNet uses for a comparably shallow design. A stem of this size, trained on 156 clips split across 25 leave-one-subject-out folds, may be overparameterised for its input in a way the smaller inputs used elsewhere in the literature are not.
...
**1. Training data.** Every row in Table 5.8 is produced by a model trained on the MEGC composite database — SMIC, CASME II and SAMM pooled together, 442 samples across three corpora and 68 subjects — and then scored on the pooled predictions restricted to the CASME II portion. §3.1.6 makes this point directly. This study trains each fold on CASME II alone: 156 clips, one corpus, 25 subjects. The published models therefore train on roughly 2.8 times the sample count of this study's single-corpus folds, drawn from three capture conditions rather than one. A cross-corpus model that has seen SAMM's 32 participants and 13 ethnicities, and SMIC's independent recording apparatus, carries a regularising effect that a single-corpus model trained on 156 clips cannot have. This difference favours the published figures.

**2. Sample count.** MEGC reports its CASME II subset as **145** samples; this study's working set is **156**. §3.1.3 traces the difference to the label taxonomy: MEGC constructs its CASME II subset from the paper's five-category release, in which sadness and fear are absorbed into "others", whereas this thesis uses the seven-label coding and assigns sadness (7 clips) and fear (2 clips) to Negative, following MEGC's own class definition applied to the finer-grained labels. Nine of the eleven-clip difference are these sadness and fear clips; the residual two clips are unaccounted for even after that adjustment, and §3.1.3 leaves them unresolved rather than forcing a reconciliation. The evaluation sets underlying the two sets of figures are consequently not identical populations, independent of the training-data difference above.

**3. The protocol limitation.** §4.6.2 records that this study's evaluation has no inner validation split: within each fold, the checkpoint is selected by its score on the held-out subject's own clips, and that same checkpoint is then scored again on those same clips. This is an optimistic bias of unknown magnitude in every fold, and therefore in the aggregate pooled macro F1 reported for every configuration, including `config_2`'s 0.7122. This point must be read in the opposite direction from the first two: it does not excuse the gap to the published figures, it widens it. If this study's own reported score is inflated by an unknown amount relative to what a genuine held-out evaluation would produce, then the true distance between `config_2` and the published methods in Table 5.8 is, if anything, larger than 0.7122 versus 0.7026–0.8764 suggests. It would be dishonest to cite only the two reasons that make this study's task harder than MEGC's without also stating the one that makes this study's own reported number more favourable than its protocol supports.
...
Four findings follow, each stated at the strength the data actually support.
...
§5.1.2 already states the result plainly: the proposed full system, `config_8_proposed_unified`, is not this study's best configuration. `config_2_temporal_only`, a transformer applied to the motion tensor alone, scores 0.7122 pooled macro F1 against `config_8`'s 0.6659 — a difference of **0.0463** in `config_2`'s favour — while `config_8` leads on pooled accuracy by exactly one clip out of 156, 117 against 116 (§5.1.2, §5.6.4).
...
Each limitation below has already been established at the point where it first became relevant to a specific result. They are gathered here because their combined effect bears on every number in this chapter at once, not only on the individual comparisons where each was first raised.
...
- **156 clips, 25 subjects, one corpus, one demographic.** §3.1.7 catalogues what this bounds: a single ethnicity and age band, laboratory recording conditions, fold sizes varying by a factor of thirty, and no basis for a claim extending beyond CASME II.
