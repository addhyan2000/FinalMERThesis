...
Single thesis-wide reference list. **Cite by key**, never by number: write `[@yan2014]` in a section file, or `[@yan2014; @qu2016]` for several. The build script assigns numbers by first appearance across the thesis and renders them in the assembled chapters, so nothing ever has to be renumbered by hand.

Adding a source: append an entry below with a new key, then cite the key. Keys are lowercase surname + year, with an initial where surnames collide (`liX2018` vs `liY2018`) and the year suffix the entry itself carries (`liong2019a`).

**Policy.** Every source listed is a paper held in the project's `docs/` corpus. Works cited only *inside* those papers — Ekman and Friesen on nonverbal leakage, the Active Shape Model and Local Weighted Mean methods, Wu et al. and Wadhwa et al. on magnification, Farnebäck and TV-L1, Pfister et al. on TIM, Newton interpolation, Lin et al. on focal loss, the SE/CBAM/ECA/GC/SRM attention modules, GoogLeNet/VGG16/ResNet-101/DenseNet-169, the CASME (2013), Polikovsky (2009) and Dual-Inception (2019) papers, and the CIFAR, ImageNet-21k and JFT-300M datasets — are attributed in the text to the reviewed source that reports them and are deliberately **not** listed here. Standard machine-learning background with no `docs/` source is stated uncited.
