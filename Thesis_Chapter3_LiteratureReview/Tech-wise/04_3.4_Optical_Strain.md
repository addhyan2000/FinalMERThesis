...
| Strain has a numerical range unlike raw flow [@liong2016] | Each channel is **min–max normalised independently** to $[0,1]$ across the sequence, so the strain channel cannot dominate training |
| Every reviewed method pools strain over time [@liong2014a; @liong2014b; @liong2016; @liong2019b] | **Temporal pooling is deliberately not applied.** The strain field is retained as a per-frame-pair sequence, since the temporal evolution of deformation is what the temporal encoder under study is meant to model |
