...
**No study reports the interaction between corrections.** Xia et al. [@xia2020a] apply augmentation and a balanced loss together; Zhao et al. [@zhaoS2021] apply focal loss alongside a large per-sample augmentation, but without class-proportional resampling. Nothing in the corpus examines whether combining a data-level and a loss-level correction over-corrects — which is the failure this project encountered and documents in §3.9.7.
...
| Balanced presentation of classes during training [@xia2020a] | Minority oversampling via a weighted sampler, active in every configuration |
